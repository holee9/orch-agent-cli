...
**2차 검증(Phase 3) 준비 완료.** 발견 버그 11개 중 7개 즉시 수정, 4개 Phase 3 이관.
...
> **Phase 1 E2E 검증 완료 (2026-03-14):** run_agent.sh + 3개 에이전트 실제 실행 확인. Phase 2 진입 준비 완료.
...
### Phase 3 — 강화 및 운영 최적화 (현재 진행 중)

**목표:** 안정성·관측성·환경 자동화·워크플로우 개선

#### P1 — 안정성 (최우선)
- [ ] **agents hot-reload**: agents.json 매 폴링 재로드 또는 SIGHUP 지원 (D-011)
- [ ] **PID lock file**: 오케스트레이터 단일 실행 보장 `/tmp/orchestrator.pid` (D-003)

#### P2 — 관측성
- [ ] **로그 시간 UTC 통일**: T1(Python logging KST) ↔ T2~T4(bash UTC Z) 불일치 수정
- [ ] T3/T4 sleeping 정상 동작 검증 완료 ✅ (신규 TASK 진입 시 자동 활성화)

#### P3 — 환경 자동화
- [ ] **AI CLI 가용성 체크**: 시작 시 `claude/codex/gemini --version` 확인 후 미설치 agent 비활성화
- [ ] **T1~T4 배당 자동 설정**: 가용 agent 기반 단계 재배분 (D-004)
- [ ] **툴체인 사전 체크**: BRIEF 스택 파싱 → 필요 CLI 검증 후 경고

#### P4 — 워크플로우
- [ ] **브랜치 자동 생성**: TASK 시작 시 T1이 `feat/TASK-NNN` 브랜치 생성 (D-005)
