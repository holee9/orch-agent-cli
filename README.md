...
3. [아키텍처](#2-아키텍처)
4. [에이전트 역할](#3-에이전트-역할)
5. [Task 라이프사이클](#4-task-라이프사이클)
6. [개발 프로세스 흐름](#5-개발-프로세스-흐름)
7. [Consensus 엔진](#6-consensus-엔진)
8. [디렉토리 구조](#7-디렉토리-구조)
9. [구현 Roadmap](#8-구현-roadmap)
10. [위험 요소 및 대응](#9-위험-요소-및-대응)
11. [Demo 시나리오](#10-demo-시나리오)
...
| `docs/agents/AGENTS.md` | 모든 AI 에이전트 | 영어 | 에이전트 공통 규칙 (task claim, 보고서 형식, 금지 사항) | (초안) |
...
| `docs/multi-ai-orchestration-implementation-plan-v2.md` | 인간 개발자 | 한국어 | 상세 설계 참고 문서 | 참고 |
...
- 에이전트 문서(`docs/agents/AGENTS.md`, `docs/agents/CLAUDE.md`, `docs/agents/CODEX.md`, `docs/agents/GEMINI.md`) 구조 변경
...
에이전트 지시 파일(`docs/agents/AGENTS.md`, `docs/agents/CLAUDE.md`, `docs/agents/CODEX.md`, `docs/agents/GEMINI.md`)은 다음 원칙을 따릅니다:

- **언어**: 반드시 영어로 작성 (에이전트는 영어 컨텍스트에서 최적 동작)
- **최소 컨텍스트**: 불필요한 배경 설명, 아키텍처 설명 제거 — "무엇을", "어떻게"만 포함
...
├── docs/
│   ├── agents/                        # 에이전트 지시 문서 (기획 초안; 구현 완료 후 루트로 승격)
...
- [x] `docs/agents/GEMINI.md` 작성 (테스트 강화 + Red Team 역할, 초안)
...
| v2.1 | 2026-03-14 | 에이전트 지시 문서를 루트에서 `docs/agents/`로 이동 (기획 초안 단계 명확화) |
| v2.2 | 2026-03-14 | `multi-ai-orchestration-implementation-plan-v2.md`를 루트에서 `docs/`로 이동 (기획 초안 문서 위치 일관성) |
