...
### 3차 검증 (BUG-2/3/P4 수정 교차검증): ✅ 완료 (2026-03-15)

2차 검증에서 발견된 버그 3건(BUG-2, BUG-3, P4)을 수정 후 라이브 파이프라인으로 교차검증했습니다.
SPEC: [SPEC-P3-003](.moai/specs/SPEC-P3-003/spec.md)

**검증 시나리오:** Issue #8 `JWT token validation middleware` (TASK-008, holee9/orch-test-target)

| 항목 | 수정 내용 | 라이브 검증 근거 |
|------|----------|----------------|
| **BUG-2** (환경제약 오탐) | `AgentVote.environment_constrained` 필드 추가 — 제약 투표를 분산 계산에서 제외 | 로그: `excluding environment-constrained agents ['claude']` (09:18:51) ✅ |
| **BUG-3** (중복 에스컬레이션) | `_escalated_task_keys` set으로 동일 agent/task 중복 방지 | Issue #8 → escalation #9 **1건만** 생성 (수정 전: #6+#7 2건) ✅ |
| **P4** (좀비 프로세스) | `start_t1.sh` pgrep + T/Z 상태 감지 후 `kill -9` | `verify_fixes.py` 16/16 통과, `bash -n` 문법 통과 ✅ |

**추가 발견 사항 (구조적 갭):**

> consensus `rework` 판정 후 오케스트레이터가 자동으로 에이전트에게 재할당하지 않아 파이프라인 중단됨.
> 수동 주입 없이 진행 불가. → **SPEC-P3-004**로 추적.

| 구분 | 수치 |
|------|------|
| 단위 테스트 추가 | 11건 (BUG-2 ×4, BUG-3 ×3, P4 ×4) |
| 실증검증 시나리오 | 16건 (`verify_fixes.py`) |
| 라이브 파이프라인 단계 | kickoff → requirements → planning → implementation(48K tok) → review → testing → consensus → re_review → escalate |
...
> **3차 검증 완료 (2026-03-15):** BUG-2/BUG-3/P4 수정 교차검증 완료. 구조적 갭 1건 신규 발견 (re-review 자동 재할당 부재). 상세 계획: [`docs/improvement-plan-v2.md`](docs/improvement-plan-v2.md)

**P1 — 완료 ✅**
- [x] `setup_labels.py` 누락 라벨 3개 추가
- [x] BUG-3: `_create_escalation_issue()` 중복 생성 방지 (`_escalated_task_keys` set) — [SPEC-P3-003](.moai/specs/SPEC-P3-003/spec.md)
...
**P3 — 완료 ✅ (2026-03-17)**
- [x] **SPEC-P3-005**: `readme-sync` 파이프라인 스테이지 구현 — `release → readme-sync → final-report` 자동 전환, append-only README 업데이트, git 커밋 자동화
- [x] **버그 수정 3건** (Live Verification issue #50, TASK-050):
  - `_advance_workflow` stale completion 경쟁 조건 수정 (오케스트레이터 재시작 시 readme-sync 스킵 방지, 타임스탬프 가드)
  - `_trigger_readme_sync` context sub-dict 누락 수정 (spec_path 등 final-report로 정상 전달)
  - `check_consensus_ready` rework 미트리거 수정 (`_trigger_rework()` 호출 누락, clear_assignment 조건 개선)
- [x] **4차 Live Verification 준비 완료**: README.md 46→50 lines, commit `f40048d docs: readme-sync for TASK-050`, 221개 테스트 전부 통과 (v3.9에서 진행)

**P3 — 후속 안정화 진행 중 (2026-03-19 부터)**
- [x] **consensus vocabulary 정규화**: approved/pass/accept → ready 키워드 정규화, 에이전트별 응답 어휘 불일치 방지 (`scripts/consensus.py`)
- [x] **AGENTS.md 인프라 파일 삭제 방지**: `templates/AGENTS.md`에 보호 규칙 추가 — 에이전트가 `.orchestra/`, `start_t*.sh`, `config/` 등 인프라 파일 삭제 금지 명시
- [x] **orchestrator 통합 테스트 48건 추가**: `test_orchestrator.py` — TASK 생애주기, 단계 전환, 에러 복구 시나리오 커버 (총 221개)
...
| v3.8 | 2026-03-17 | **SPEC-P3-005 readme-sync 구현 완료 및 4차 Live Verification:** `release → readme-sync → final-report` 자동 전환, append-only README 업데이트·git 커밋 자동화, stale completion 경쟁 조건·context 누락·`_trigger_rework` 미호출 버그 3건 수정, 221개 테스트 통과 |
| v3.9 | 2026-03-19 | **SPEC-P3-005 후속 안정화:** consensus vocabulary 정규화(approved/pass/accept→ready), AGENTS.md 인프라 파일 삭제 방지 보호 규칙, orchestrator 통합 테스트 48건 추가(총 221개), 4차 Live Verification 진행 중 |
