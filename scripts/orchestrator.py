...
class Orchestrator:
    ...
    def __init__(self, config_path: str | Path = "config/config.yaml"):
        ...
        orchestra_dir = Path(target_path) / self.config["orchestrator"]["orchestra_dir"] if target_path else Path(self.config["orchestrator"]["orchestra_dir"])
    ...
    def _advance_workflow(self, agent_id: str, task_id: str, completion: dict) -> None:
        ...
        if next_stage:
            ...
            if next_agent:
                ...
                logger.info("Advanced %s: %s -> %s (agent: %s)", task_id, current_stage, next_stage, next_agent)
    ...
    def check_consensus_ready(self) -> int:
        ...
        for task_id, issue_number in review_tasks.items():
            ...
            if all(agent_id in reports for agent_id in review_agents):
                votes = self.consensus_engine.build_votes_from_reports(reports, self.agents)
                ...
                if result.can_proceed:
                    self.github.update_labels(issue_number, add=["consensus:pass"], remove=["status:review-needed"])
                elif result.action == "escalate":
                    self.github.update_labels(issue_number, add=["escalation:human"], remove=["status:review-needed"])
                else:
                    self.github.update_labels(issue_number, add=["consensus:fail"], remove=["status:review-needed"])
